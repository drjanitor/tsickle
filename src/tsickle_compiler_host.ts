...
export class TsickleCompilerHost implements ts.CompilerHost {
  ...
  stripAndStoreExistingSourceMap(sourceFile: ts.SourceFile): ts.SourceFile {
    if (sm.containsInlineSourceMap(sourceFile.text)) {
      ...
      const stripedSourceText = sm.removeInlineSourceMap(sourceFile.text);
      const stripedSourceFile =
          ts.createSourceFile(sourceFile.fileName, stripedSourceText, sourceFile.languageVersion);
      this.sourceFileToPreexistingSourceMapMap.set(stripedSourceFile, sourceMap);
  ...
  combineSourceMaps(filePath: string, tscSourceMapText: string): string {
    ...
    // loaded.
    if (this.sourceFileToPreexistingSourceMapMap.size > 0 &&
        this.preexistingSourceMaps.size === 0) {
      this.sourceFileToPreexistingSourceMapMap.forEach((sourceMap, sourceFile) => {
